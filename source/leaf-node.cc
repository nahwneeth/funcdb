...
LeafNode::LeafNode(std::size_t index, std::fstream& fstream,
  ...
  for (std::size_t i = 0; i < numElems; ++i) {
    ...
    mRecords.push_back(std::move(elem));
  }
}
...
std::unique_ptr<char[]> LeafNode::Serialize() const {
  ...
  std::size_t numElems = mRecords.size();
  ...
  for (std::size_t i = 0; i < numElems; ++i) {
    std::memcpy(buffer.get() + offset, &mRecords[i].key, keySize);
    offset += keySize;

    std::memcpy(buffer.get() + offset, mRecords[i].value.get(), gValueSize);
    offset += gValueSize;
  }
...
void LeafNode::Insert(std::size_t index, Record& elem) {
  mRecords.insert(std::next(mRecords.begin(), index), std::move(elem));
}
...
LeafNode::SearchResult LeafNode::Search(int32_t key) const {
  ...
  while (l <= u) {
    ...
    if (mRecords[m - 1].key == key) {
      res.exists = true;
      res.index = m - 1;
      return res;
    } else if (mRecords[m - 1].key < key)
      l = m + 1;
    else
      u = m - 1;
  }
